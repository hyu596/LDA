...
# Config variables
host = "172.31.9.132" # set to IP address of target computer
port = 13000
addr = (host, port)
...
while True:
    ...
    if time.time() - start_time_local > 200: break
    ...
    if i%10 == 0:
        ...
        scores += [score]
        times += [time.time() - start_time_local]
...
#np.save('results/n=10,p=1,time=200/logli/client'+str(client_id), scores)
#np.save('results/n=10,p=1,time=200/time/client'+str(client_id), times)
